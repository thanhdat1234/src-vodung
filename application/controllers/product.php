...

{
...

    {
        ...
        $prod_options = $this->db->from('product_options')->order_by('order','ASC')->get()->result_array();
        ...
        $data['data']['prod_options'] = $prod_options;
...

    {
        ...
        if (!empty($product) && count($product)) {
            ...
            $sls_config = $this->db->from('products_config')->where('product_id', $id)->get()->result_array();
            ...
            $data['data']['_prd_config'] = $sls_config;
    ...
    public function cms_create_config()

    {
        ...
        $prd_config = $this->db->from('products_config')->where(['prd_config_name' => $data['prd_config_name'], 'prd_config_value' => $data['prd_config_value']])->get()->result_array();

        if (!empty($prd_config) && count($prd_config)) {
            ...
            $this->db->insert('products_config', $data);
    ...
    public function cms_paging_config($page = 1)

    {

        $config = $this->cms_common->cms_pagination_custom();

        $total_prdconfig = $this->db->from('products_config')->count_all_results();

        $config['base_url'] = 'cms_paging_config';

        $config['total_rows'] = $total_prdconfig;
        ...
        $data['_list_prd_config'] = $this->db->from('products_config')->limit($config['per_page'], ($page - 1) * $config['per_page'])->order_by('created_at', 'desc')->get()->result_array();

        if ($page > 1 && ($total_prdconfig - 1) / ($page - 1) == 10)
        ...
        $this->load->view('ajax/product/list_prd_config', isset($data) ? $data : '');
    ...
    public function cms_delete_config($id)

    {

        $id = (int)$id;

        $prd_config = $this->db->from('products_config')->where('ID', $id)->get()->row_array();

        if (!isset($prd_config) || count($prd_config) == 0) {
            ...
            $this->db->where('ID', $id)->delete('products_config');
    ...
    public function cms_update_prdconfig($id)

    {
        ...
        if (!empty($prd_config) || count($prd_config) != 0) {

            $check = $this->db->from('products_config')->where(['prd_config_name' => $data['prd_config_name'], 'prd_config_value' => $data['prd_config_value']])->get()->result_array();

            if (empty($check) && count($check) == 0) {

                $data['updated'] = gmdate("Y:m:d H:i:s", time() + 7 * 3600);

                $data['user_update'] = $this->auth['id'];

                $this->db->where('ID', $id)->update('products_config', $data);
...
?>
    ...
    public function cms_load_listconfig()

    {

        $this->cms_nestedset->set('products_group');

        $data = $this->db->from('products_config')->order_by('created_at', 'desc')->get()->result_array();

        ob_start();

        echo '<option value="-1" selected="selected">--Cấu hình--</option>';

        echo '<optgroup label="Chọn cấu hình">';

        foreach ($data as $key => $item) :

        ?>

            <option value="<?php echo $item['ID']; ?>"><?php echo $item['prd_config_name'] . ':' . $item['prd_config_value']; ?>
        ...
        <option value="product_config" data-toggle="modal" data-target="#list-prd-config">Tạo mới cấu hình
...

    {
        ...
        ?>
            ...
            <option value="<?php echo $item['ID']; ?>"><?php echo $item['prd_unit_name']; ?>
...
<?php
...

    {
        ...
        if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if (isset($_POST["linkImg"]) && is_array($_POST["linkImg"])) {

                foreach ($_POST["linkImg"] as $key => $value) {
                    ...
                    if (in_array($ext, $valid_formats)) {
                        ...
                        if ($size < (10024 * 10024)) {
                            ...
                            if (move_uploaded_file($tmp, $path . $image_name)) {
                                ...
                                if ($i == count($_FILES['photos']['name']) - 1) {
...

    {
        ...
        $dataConfigName = $data['config_name'];

        $dataConfigValue = $data['config_value'];

        $linkImg = '';

        $dataLinkImg = $data['link_img'];

        for ($i = 0; $i < count($dataLinkImg); $i++) {

            if ($dataLinkImg[$i]) {

                $image_name = time() . $i . ".jpg";

                file_put_contents('public/templates/uploads/' . $image_name, file_get_contents($dataLinkImg[$i]));

                if($i == 0) {

                    $linkImg = $linkImg. $image_name;
        ...
        if($data['prd_image_url']) {
            ...
            $data['prd_image_url'] = $data['prd_image_url'] .','. $linkImg;
        ...
        if (!empty($check_code) && count($check_code)) {
            ...
            for ($i = 0; $i < count($dataConfigName); $i++) {

                if ($dataConfigName[$i] && $dataConfigValue[$i]) {

                    $config = ['product_id' => $product_id, 'prd_config_name' => $dataConfigName[$i], 'prd_config_value' => $dataConfigValue[$i], 'user_init' => $user_init];

                    $this->db->insert('products_config', $config);
            ...
            $result = $this->db->insert('report', $report);
...

    {
        ...
        if ($data['prd_image_url']) {
            ...
            $data['prd_image_url'] = $data['prd_image_url'];
        ...
        for ($i = 0; $i < count($dataConfigName); $i++) {
            ...
            if ($dataConfigId[$i] == 0) {
                ...
                if ($dataConfigName[$i] && $dataConfigValue[$i]) {
                    ...
                    $config = ['product_id' => $id, 'prd_config_name' => $dataConfigName[$i], 'prd_config_value' => $dataConfigValue[$i], 'user_init' => $this->auth['id']];
                ...
                if ($dataConfigName[$i] && $dataConfigValue[$i]) {
                    ...
                    $config = ['prd_config_name' => $dataConfigName[$i], 'prd_config_value' => $dataConfigValue[$i], 'user_update' => $this->auth['id']];

                    $this->db->where('ID', $dataConfigId[$i])->update('products_config', $config);
...

    {
        ...
        if (!empty($product) && count($product)) {
            ...
            $data['_detail_product']['_prd_config'] = $sls_config;
