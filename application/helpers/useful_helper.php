...
if (!function_exists('cms_getNameConfigbyID')) {
    function cms_getNameConfigbyID($id)
    {
        $name = 'Chưa có';
        $CI =& get_instance();
        $config = $CI->db->select('prd_config_name,prd_config_value')->from('products_config')->where('ID', $id)->get()->row_array();
        if (isset($config) && count($config)) {
            $name = $config['prd_config_name'].':'. $config['prd_config_value'];
        }
